...
def plot(datasets, input_labels, output_label, plot_output_dir, dataset_name_on_rows=False, size=60):
    """
    ...
    for label in input_labels:
        ...
        fig, axs = plt.subplots(num_samples, num_datasets, figsize=(50, 50), subplot_kw={'xticks': [], 'yticks': []},
        ...
        for j, dataset_name in enumerate(sorted(datasets['simulation'].keys())):
            if dataset_name_on_rows:
                axs[0, j].set_title(dataset_name, {'family': 'serif', 'color': 'black', 'size': size})
            input_sample = datasets['simulation'][dataset_name][label]
            gt_sample = datasets['simulation'][dataset_name][output_label]
            input_norm_sample, gt_norm_sample = np.log1p(input_sample), np.log1p(gt_sample)
...
def main():
    ...
    for model_name, dataset in cfg.dataset.paths.items():
        ...
        for dataset_name in dataset.keys():
            mat_files = read_mat_files(dataset[dataset_name])[0]
            datasets[dataset_name] = prepare_label_to_idx_mapping_for_analysis(mat_files)
    print("Plotting Vis.")
    plot(datasets, cfg.dataset.input_labels, cfg.dataset.output_label, cfg.output_dir, cfg.dataset_name_on_rows,
         cfg.size)
    print("Done plotting.")
