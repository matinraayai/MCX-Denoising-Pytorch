...
class OsaDataset(tf.data.Dataset, ABC):
    ...
    def _generator(root: str, input_file_name: str, label_file_name: str, num_samples: int):
        for i in range(num_samples):
            x = spio.loadmat(root + input_file_name.format(i), squeeze_me=True)['data']
            y = spio.loadmat(root + label_file_name.format(i), squeeze_me=True)['data']
