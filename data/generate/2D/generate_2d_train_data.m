function generate_2d_train_data(photon_list, n, overwrite, shape_range, img_dims, output_dir, gpu_ids)
    ...
    for i = 1 : n
        ...
        if length(shape_range) == 1
            num_shapes = shape_range;
        else
            num_shapes = randi(shape_range);
        end
        % Configuration used for all the simulations in the file
        cfg = generate_random_2d_cfg(img_dims, num_shapes, gpu_ids);
